...
if (isset($_GET['id_akses_login'])) {
	...
	$status_hapus = $lib->deleteAdmin($kd_admin);
	if ($status_hapus) {
		echo "<script type='text/javascript'>alert('Berhasil Menghapus Data Admin!');window.location.href = 'tabel_admin.php';</script>";
	} else {
...
<head>
	...
	<title>SPPDB - Informasi Admin</title>
...
<body class="hold-transition sidebar-mini">
					...
					<ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
						...
						<li class="nav-item has-treeview">
							...
							<ul class="nav nav-treeview">
								...
								<li class="nav-item">
									<a href="tabel_admin.php" class="nav-link bg-cyan">
						...
						<div class="col-12">
							<div class="card">
								<div class="card-header">
									<h3 class="card-title">Tabel Data Admin <a target="_blank" href='print_data_admin.php' class='btn btn-default'><i class='fa fa-print'></i> Print</a> <a href='javascript:exportData()' class='btn btn-success'><i class='fas fa-file-excel'></i> Export Excel</a></h3>
								</div>
								<!-- /.card-header -->
								<div class="card-body">
										...
										<table id="dtHorizontalVerticalExample" class="table table-bordered table-striped" cellspacing="0" width="100%">
											...
											<tbody>
												...
												foreach ($data_admin as $row) {
													...
													echo "<td><a href='form_edit_admin.php?id_akses_login=" . $row['id_akses_login'] . "' class='btn btn-info'><i class='fas fa-edit'></i> Update</a>
											<a href='tabel_admin.php?id_akses_login=" . $row['id_akses_login'] . "' class='btn btn-danger'><i class='fa fa-trash'></i> Hapus</button></td>";
													echo "</tr>";
													$no++;
												}
												?>
	...
	<script type="text/javascript">
		function exportData() {
			if (confirm("Apakah anda ingin export Data Admin?")) {
				window.location.href = 'export_excel_admin.php';
			} else {
				alert('Batal export Data Admin!');
				window.location.href = 'tabel_admin.php?export=batal';
			}
		}
		$(function() {
			$("#example1").DataTable({
