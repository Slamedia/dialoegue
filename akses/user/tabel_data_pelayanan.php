...
if (isset($_GET['hapus_pelayanan'])) {
	...
	if ($status_hapus) {
		...
		echo "<script type='text/javascript'>alert('Gagal Menghapus Data Pelayanan, Silahkan Hubungi Admin Melalui Kontak Kami :)');window .location.href = 'tabel_data_pelayanan.php';</script>";
	}
}
?>
<!DOCTYPE html>
...
<head>
	...
	<title>SPPDB - Informasi Pelayanan KTP & KK</title>
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1>Informasi Pelayanan Kartu Keluarga & KTP</h1>
						</div>
						<div class="col-sm-6">
										...
										<table id="dtHorizontalVerticalExample" class="table table-bordered table-striped" cellspacing="0" width="100%">
											<thead>
												<tr>
													...
													<th>Keterangan</th>
													<th>Lampiran Surat Permohonan Kartu Keluarga</th>
													<th>Action</th>
												</tr>
											</thead>
											<tbody>
												...
												foreach ($data_pelayanan as $row) {
													...
													if (
														$row['status_proses_pengajuan'] == 'Dikirim'
													) {
														$lampiran = '<font color="red">Lampiran Tidak Tersedia</font>';
													} else if (
														$row['status_proses_pengajuan'] == 'diproses'
													) {
														$lampiran = '<font color="red">Lampiran Sedang Di Proses</font>';
													} else if (
														$row['status_proses_pengajuan'] == 'batal'
													) {
														$lampiran = '<font color="red">Lampiran Belum Tersedia dan Proses Pelayanan Dibatalkan</font>';
													} else if (
														$row['status_proses_pengajuan'] == 'selesai'
													) {
														if ($row['keterangan_pelayanan'] == 'Membuat KTP Baru') {
															$tgl1 = $row['tanggal_proses']; // pendefinisian tanggal awal
															$tgl2 = date('Y-m-d', strtotime('+6 days', strtotime($tgl1))); //operasi penjumlahan tanggal sebanyak 6 hari
															if ($tgl1 > $tgl2) {
																$lampiran = '<font color="red">Lampiran Terlewat Batas</font>';
															} else {
																$lampiran = "<a class='btn btn-info' href='../../berkas/surat_permohonan/surat_permohonan_ktp.php?kode_nik=" . $row['nik_penduduk'] . "'><i class='fa fa-print'></i>Lampiran Tersedia</a>";
															}
														} else if ($row['keterangan_pelayanan'] == 'Membuat KTP Baru(Kehilangan)') {
															$tgl1 = $row['tanggal_proses']; // pendefinisian tanggal awal
															$tgl2 = date('Y-m-d', strtotime('+6 days', strtotime($tgl1))); //operasi penjumlahan tanggal sebanyak 6 hari
															if ($tgl1 > $tgl2) {
																$lampiran = '<font color="red">Lampiran Terlewat Batas</font>';
														...
														if ($row['keterangan_pelayanan'] == 'Membuat Kartu Keluarga Baru(Kehilangan)' || $row['keterangan_pelayanan'] == 'Membuat Kartu Keluarga Baru' || $row['keterangan_pelayanan'] == 'Membuat Kartu Keluarga Baru(Tambah Anggota)') {
															$tgl1 = $row['tanggal_proses']; // pendefinisian tanggal awal
															$tgl2 = date('Y-m-d', strtotime('+6 days', strtotime($tgl1))); //operasi penjumlahan tanggal sebanyak 6 hari
															if ($tgl1 > $tgl2) {
																$lampiran = '<font color="red">Lampiran Terlewat Batas</font>';
															} else {
																$lampiran = "<a class='btn btn-info' href='../../berkas/surat_permohonan/surat_permohonan_kk.php?kode_nik=" . $row['nik_penduduk'] . "'><i class='fa fa-print'></i> Lampiran Tersedia</a>";
															}
														}
													}
													...
													echo "<td>" . $lampiran . "</td>";
													echo "<td><a href='tabel_data_pelayanan.php?hapus_pelayanan=" . $row['id_pelayanan'] . "' class='btn btn-danger'>Hapus</a></td>";
													echo "</tr>";
													$no++;
												}
												?>
